...
["change", "keyup", "input", "paste"].forEach((event) => elements.search.addEventListener(event, search));
...
function generateWord(result) {
	return `<div class="word-box"><span class="word">${result.word}</span>${result.note ? `<span class='note'>${result.note}</span>` : ""}<div class="translation-container"><span class="translation">${["pf", "ppp", "ppa", "pfa"].includes(result.note) ? "→ " : ""}${result.translations.join(
		"; "
	)}</span></div></div>`;
}
